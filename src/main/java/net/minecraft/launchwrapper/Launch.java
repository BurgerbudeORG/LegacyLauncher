...
public class Launch {
  ...
  /**
   * The game directory of Minecraft. Changing this will change the game directory value passed to
   * tweakers.
   */
  public static File minecraftHome;

  /**
   * The asset directory of Minecraft. Changing this will change the assets directory value passed
   * to tweakers.
   */
  public static File assetsDir;

  /**
   * The key-value blackboard.
   */
  public static Map<String, Object> blackboard;
  /**
   * The class loader to register transformers to. Changing this will change the class loader passed
   * to tweakers.
   */
  ...
  private void launch(String[] args) {
    ...
    final OptionSpec<File> gameDirOption =
        parser
            .accepts("gameDir", "Alternative game directory")
            .withRequiredArg()
            .ofType(File.class);
    final OptionSpec<File> assetsDirOption =
        parser.accepts("assetsDir", "Assets directory").withRequiredArg().ofType(File.class);
    final OptionSpec<String> tweakClassOption =
        parser.accepts("tweakClass", "Tweak class(es) to load").withRequiredArg();
    ...
    try {
      ...
      allTweakers.stream()
          .map(tweaker -> Arrays.asList(tweaker.getLaunchArguments()))
          .forEach(argumentList::addAll);

      // Finally we turn to the primary tweaker, and let it tell us where to go to launch
      final String launchTarget = Objects.requireNonNull(primaryTweaker).getLaunchTarget();
      ...
      LOGGER.error("Unable to launch", exception);
      System.exit(1);
    }
  }
}
